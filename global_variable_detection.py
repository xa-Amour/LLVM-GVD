...
_GLOBAL_VAR_COUNT = 0  # records the number of global variables, including filtered
...
def write_data_to_result(analysis_file, indent, global_variable_scope, global_variable_name, global_variable_type,
                         global_variable_line, global_variable_colum, save_file=Global_Check_Result_Txt):
    try:
        with open(save_file, 'a') as fileWriter:
            data_record = '{file_name}, ' \
                          ...
                          '{global_variable_line}-{global_variable_colum}\n'. \
                format(file_name=analysis_file, \
                       indent=indent, \
                       global_variable_scope=global_variable_scope, \
                       global_variable_name=global_variable_name, \
                       global_variable_type=global_variable_type, \
                       global_variable_line=global_variable_line, \
                       global_variable_colum=global_variable_colum)
...
def check_argv():
    if len(sys.argv) != 2:
        print("Usage: global_variable_detection.py [header file name]")
        sys.exit()


def run_thread(suffixation):
    source_code = []
    source_code.append(options.source_dir)
...
def task_thread():
    ...
    start = time.time()
    empty_last_result(Global_Check_Result_Txt)
    threads = []
...
def main():
    ...
    start = time.time()
    empty_last_result([Global_Check_Result_Txt, Custom_Rules_Report_Xml])
    main_processing()
    post_processing(Global_Check_Result_Txt)  # remove the items by sonar.exclusions in sonar-project.properties
    end = time.time()
    logging.info("There are [%s] global variables in this scan" % _GLOBAL_VAR_COUNT)
    logging.info("Running time of this program is %.2f" % (end - start))
